...
export default class Bound {
  ...
  public isOverlaped(bound: Bound): boolean {
    ...
    return (
      Math.abs(center1[0] - center0[0]) < (this.width + bound.width) / 2 &&
  ...
  public contain(a: any): boolean {
    if (a instanceof Bound) {
      ...
      for (let i = 0; i < vertexs.length; i++) {
        if (!this.contain(vertexs[i])) return false
      }
      return true
    }
  ...
  public isIntersectOfSegment(p1: Vertex, p2: Vertex): boolean {
    const bound = Algorith.getBoundOfPoints([p1, p2])
    if (!bound.isOverlaped(this)) {
      return false
    }
    if (this.contain(p1) || this.contain(p2)) {
      return true
    }
    const base: Vector2 = Algorith.getDirectionVec(p1, p2)
    const vertexs: Vertex[] = this.getVetexs()
    for (let i = 1; i < vertexs.length; i++) {
      const preVec: Vector2 = Algorith.getDirectionVec(p1, vertexs[i - 1])
      const curVec: Vector2 = Algorith.getDirectionVec(p1, vertexs[i])
      if (Algorith.isBothSide(base, preVec, curVec)) {
        return true
      }
    }
    return false
