...
export default class CandlestickWidget extends IWidget {
  public config = { strokeStyle: '#242424', zIndex: 1 };
  public renderer = new CandlestickRenderer();
  public render() {
    this.initWidget();
    const ctx: CanvasRenderingContext2D = this.getContext();
    const _hitCtx = this.getParent().getHitContext();
    this.setCanvasContext(ctx);
    this.setCanvasContext(_hitCtx, { strokeStyle: '#9e9e9e' });
  ...
  public onMousemove(point: Point) {
    ...
    Canvas.clearRect(_hitCtx, { ...this.bound, x: 0, y: -this.bound.height });
    this.renderer.drawCrossLine(_hitCtx, point, this.getBound());
